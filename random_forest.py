...
class RandomForest:
    ...
    def build(self, X, y):
        ...
        for tree_i in range(self.num_trees):
            ...
            # obtain random indices for data and features
            data_indices = self.rng.permutation(num_data_per_tree)
            X_sub = X[data_indices]
            y_sub = y[data_indices]
            feat_ids = self.rng.permutation(num_features_per_tree)
