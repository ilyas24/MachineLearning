...
class EMPCA():
    ...
    def _e_step(self, Y, A):
        ...
        # left matrix representing (A^TxA)^-1
        left_matrix = np.linalg.inv(A.T @ A)
        # right matrix representing A^T x Y
        right_matrix = A.T @ Y
    ...
    def _m_step(self, X, Y):
        ...
        # left matrix represents YX^T
        left_matrix = Y @ X.T 
        # right matrix represents (XX^T)^-1
        right_matrix = np.linalg.inv(X @ X.T)
