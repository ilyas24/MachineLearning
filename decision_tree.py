...
class DecisionTree:
    ...
    def _entropy(self, y):
        ...
        # get amount of class labels to obtain probability
        num_of_labels = y.shape[0]
        # this array will hold probabilities of labels (Pi)
        p_i = []
        # traverse through counts to consider probability of labels
        for num_points in counts:
            # when p_i = 0, since lim(p_ilog(p_i)) = 0 when p_i approaches 0
            # we'll store 1 since log2(1) = 0
            if (num_points == 0):
                p_i.append(1)
            else:
                p_i.append(num_points/num_of_labels)
        # represent entropy variable
        entropy = 0
        # obtain log2 of p_i to calculate entropy
        logp_i = np.log2(p_i)
        # traverse p_i to obtain entropy
        for i in range(len(p_i)):
            entropy = entropy - p_i[i]*logp_i[i]
    ...
    def _find_split(self, X, y, H_data):
        ...
        for ii in range(unique_values.shape[0] - 1):
            ...
            # entropy of data <= split value
            H_left = self._entropy(y[0:current_split_index])
            # probability of left path
            p_left = (current_split_index)/N
            # entropy of data > split value
            H_right = self._entropy(y[current_split_index:])
            # probability of right path
            p_right = (N - current_split_index)/N
            # conditional entropy considering H_right and H_left
            H_X = (p_left * H_left) + (p_right * H_right)
            current_information_gain = H_data - H_X
